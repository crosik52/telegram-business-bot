...
<b>TELETHON_SESSION_STR</b> в Replit Secrets и в Railway Variables:</p>
<div class="session">{session_str}</div>
<div class="note">
  <b>Что дальше:</b><br>
  1. Скопируй строку выше<br>
  2. Replit → Secrets → обнови <code>TELETHON_SESSION_STR</code><br>
  3. Railway → Variables → обнови <code>TELETHON_SESSION_STR</code><br>
  4. В Railway Settings → Deploy → поставь <b>Restart Policy: Recreate</b> (не Rolling)<br>
  5. Railway передеплоит автоматически
</div>""")
    except Exception as exc:
