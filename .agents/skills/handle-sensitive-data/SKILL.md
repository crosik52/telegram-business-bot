...
- **Protect in transit** (HTTPS only) **and at rest** (hash passwords, encrypt regulated data). Do not send PII to third parties (analytics, model APIs, error trackers) unless intended and covered — scrub it first.
- **Don't leak sensitive details** in error messages shown to users.
- **Build for data-subject rights**: if the app stores personal data, ensure a user's records can be found and deleted/exported by user id. Don't retain PII forever by default.
